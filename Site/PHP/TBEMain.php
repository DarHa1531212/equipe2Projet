<?php
    ...
    function VerifEvaluation($tblEvaluation, $profil){
        ...
        {
            $div = '<tr class="itemHover" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->Id.'&nomMenu=Evaluation.php\', \'&idStage=\', '.$tblEvaluation[0]->IdStage.', \'&idEvaluation=\', '.$tblEvaluation[0]->IdEvaluation.', \'&typeEval=1\');">';
        }
        else
        {
            $div = '<tr>';
        ...
        {
            $div = '<tr class="itemHover" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->Id.'&nomMenu=Evaluation.php\', \'&idStage=\', '.$tblEvaluation[1]->IdStage.', \'&idEvaluation=\', '.$tblEvaluation[1]->IdEvaluation.', \'&typeEval=2\')">';
        }
        else
        {
            $div = '<tr>';
    ...
    foreach($profils as $profil){
        ...
        <div class="infoStagiaire">
            ...
            <input class="bouton" type="button" value="Afficher le profil" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->Id.'&nomMenu=Profil.php\')"/>
            <h3>'.$profil->NumTel.'</h3>
        </div>

        <div class="blocInfo itemHover">
            <a class="linkFill" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->IdEnseignant.'&nomMenu=Profil.php\')">
                <div class="entete">
                    <h2>Enseignant</h2>
                </div>
        ...
        if($_SESSION['IdRole'] != 4){//Si l'utilisateur n'est pas superviseur, affiche les infos du superviseur.
            ...
            '
            <div class="blocInfo itemHover">
                <a class="linkFill" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->IdSuperviseur.'&nomMenu=Profil.php\')">
                    <div class="entete">
                        <h2>Superviseur</h2>
                    </div>
        ...
        if(count($profils) > 1){//Si il y a plus qu'un stagiaire, affiche les flèches.
            $content = $content.
            '<input class="bouton" type="button" value="Écrire un commentaire" onclick="Execute(1, \'../PHP/TBNavigation.php?id='.$profil->IdSuperviseur.'&nomMenu=AVenir.php\')"/>
